        ...
        return new Promise(async (resolve, reject) => {
            ...
            if (timeout === null) {
                try {
                    ...
                    timeout = setTimeout(() => {
                        console.log('pre clear, and timeout is :', timeout)
                        // 取消定时器
                        timeout = null
                        console.log('clear, and timeout is :', timeout)
                    }, delay)

                    console.log('before, and timeout is :', timeout)
                    // 由于有箭头函数来绑定 this，所以可以不用 apply来调用。
                    resolve(fn(...arguments))
                    console.log('after, and timeout is :', timeout)
